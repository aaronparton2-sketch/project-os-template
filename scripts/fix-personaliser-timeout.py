"""Fix personaliser timeout by splitting into 3 nodes:
1. Build Prompt (Code node — fast, no HTTP)
2. OpenAI API (HTTP Request node — native, no task runner timeout)
3. Parse Response (Code node — fast, no HTTP)

The Code node task runner has a 60-second TOTAL timeout. With 52 items × 2-3s per
OpenAI call, it always exceeds this. HTTP Request nodes don't use the task runner
and have no such limit — they process items one at a time natively.
"""
...
# ═══════════════════════════════════════════════════════════════════
# PIPELINE A — Build Prompt (Code node, fast, no HTTP)
# ═══════════════════════════════════════════════════════════════════
PROMPT_A_CODE = r"""// Build Email Prompt — Pipeline A (No Website)
// Fast: no HTTP calls. Just builds the prompt and passes it downstream.

const SYSTEM_PROMPT = `You are writing a cold email from Aaron, a web designer based in Perth who works with businesses across Australia.
...
# ═══════════════════════════════════════════════════════════════════
# PIPELINE A — Parse Response (Code node, fast, no HTTP)
# ═══════════════════════════════════════════════════════════════════
PARSE_A_CODE = r"""// Parse OpenAI Response — Pipeline A
// Fast: just parses the JSON response and builds final output.
...
try {
  const content = lead.choices?.[0]?.message?.content || '{}';
  const cleaned = content.replace(/```json\n?/g, '').replace(/```\n?/g, '').trim();
  emailData = JSON.parse(cleaned);
} catch {
  parseError = true;
  emailData = { subject: '[PARSE ERROR]', body: lead.choices?.[0]?.message?.content || '', call_script: '' };
}
...
# ═══════════════════════════════════════════════════════════════════
# PIPELINE B — Build Prompt (Code node, fast, no HTTP)
# ═══════════════════════════════════════════════════════════════════
...
CONTEXT: Aaron rebuilds websites for businesses that have a DIY site (Wix, Squarespace, etc.) that's slow or poorly built. He includes REAL data from their site audit. The pitch is: your site is costing you customers, I can fix it.
...
# ═══════════════════════════════════════════════════════════════════
# PIPELINE B — Parse Response (Code node, fast, no HTTP)
# ═══════════════════════════════════════════════════════════════════
...
# Remove old personaliser nodes
wf['nodes'] = [n for n in wf['nodes'] if n['name'] not in ('AI Email Personaliser', 'AI Email Personaliser1')]

# ── Pipeline A: 3 new nodes ──
tag_a_pos = positions.get('Tag & Pass All Leads', [704, -176])
log_a_pos = positions.get('Log Pipeline Run', [1280, -176])

wf['nodes'].extend([
    {
        'name': 'Build Email Prompt A',
        'type': 'n8n-nodes-base.code',
        'typeVersion': 2,
        'position': [tag_a_pos[0] + 250, tag_a_pos[1]],
        'parameters': {
            'mode': 'runOnceForEachItem',
            'jsCode': PROMPT_A_CODE,
        },
    },
    {
        'name': 'OpenAI: Draft Email A',
        'type': 'n8n-nodes-base.httpRequest',
        'typeVersion': 4.2,
        'position': [tag_a_pos[0] + 500, tag_a_pos[1]],
        'parameters': {
            'method': 'POST',
    ...
    },
    {
        ...
        'type': 'n8n-nodes-base.code',
        'typeVersion': 2,
        'position': [tag_a_pos[0] + 750, tag_a_pos[1]],
        'parameters': {
            'mode': 'runOnceForEachItem',
            'jsCode': PARSE_A_CODE,
        },
...
# ── Pipeline B: 3 new nodes ──
tag_b_pos = positions.get('Tag & Pass All Leads B', [896, 144])
log_b_pos = positions.get('Log Pipeline Run1', [1504, 144])

wf['nodes'].extend([
    {
        'name': 'Build Email Prompt B',
        'type': 'n8n-nodes-base.code',
        'typeVersion': 2,
        'position': [tag_b_pos[0] + 250, tag_b_pos[1]],
        'parameters': {
            'mode': 'runOnceForEachItem',
            'jsCode': PROMPT_B_CODE,
        },
    },
    {
        'name': 'OpenAI: Draft Email B',
        'type': 'n8n-nodes-base.httpRequest',
        'typeVersion': 4.2,
        'position': [tag_b_pos[0] + 500, tag_b_pos[1]],
        'parameters': {
            'method': 'POST',
            'url': 'https://api.openai.com/v1/chat/completions',
            'authentication': 'genericCredentialType',
    ...
    },
    {
        ...
        'type': 'n8n-nodes-base.code',
        'typeVersion': 2,
        'position': [tag_b_pos[0] + 750, tag_b_pos[1]],
        'parameters': {
            'mode': 'runOnceForEachItem',
            'jsCode': PARSE_B_CODE,
        },
...
# Remove old personaliser connections
conns.pop('AI Email Personaliser', None)
conns.pop('AI Email Personaliser1', None)

# Pipeline A: Tag → Build Prompt → OpenAI → Parse → Log
conns['Tag & Pass All Leads'] = {
    'main': [[{'node': 'Build Email Prompt A', 'type': 'main', 'index': 0}]]
}
conns['Build Email Prompt A'] = {
    'main': [[{'node': 'OpenAI: Draft Email A', 'type': 'main', 'index': 0}]]
}
conns['OpenAI: Draft Email A'] = {
    'main': [[{'node': 'Parse Email A', 'type': 'main', 'index': 0}]]
}
conns['Parse Email A'] = {
    'main': [[{'node': 'Log Pipeline Run', 'type': 'main', 'index': 0}]]
}

# Pipeline B: Tag → Build Prompt → OpenAI → Parse → Log
conns['Tag & Pass All Leads B'] = {
    'main': [[{'node': 'Build Email Prompt B', 'type': 'main', 'index': 0}]]
}
conns['Build Email Prompt B'] = {
    'main': [[{'node': 'OpenAI: Draft Email B', 'type': 'main', 'index': 0}]]
}
conns['OpenAI: Draft Email B'] = {
    'main': [[{'node': 'Parse Email B', 'type': 'main', 'index': 0}]]
}
conns['Parse Email B'] = {
    'main': [[{'node': 'Log Pipeline Run1', 'type': 'main', 'index': 0}]]
}

# ═══════════════════════════════════════════════════════════════════
# HANDLE AUTH — Try to find or create httpHeaderAuth credential
# We can't use openAiApi credential type with HTTP Request nodes.
# Instead, we inject the API key directly via header parameters.
# ═══════════════════════════════════════════════════════════════════

# Remove credential references from HTTP nodes (use direct header instead)
for node in wf['nodes']:
    if node['name'] in ('OpenAI: Draft Email A', 'OpenAI: Draft Email B'):
        node.pop('credentials', None)
        # Remove genericAuth params — use direct header auth
...
print('Restructured workflow:')
print('  Pipeline A: Tag -> Build Prompt A -> OpenAI A -> Parse A -> Log')
print('  Pipeline B: Tag -> Build Prompt B -> OpenAI B -> Parse B -> Log')
